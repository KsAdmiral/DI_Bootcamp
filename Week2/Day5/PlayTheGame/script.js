...
function playTheGame() {
  const pcNumber = compRandomNumber();
  let didWin = false;
  let retries = 3;
  let confirmResult = confirm("Would you like to play?");
  while (!didWin && retries !== 0) {
    retries--;
    if (!confirmResult) {
    ...
    let userNumber;
    let userInput = prompt("Choose a number between 0-10");
    ...
    if (isNaN(userNumber)) {
      ...
      didWin = compareNumbers(userNumber, pcNumber, retries);
    }
  }
}

function compareNumbers(userNumber, computerNumber, retries) {
  if (userNumber === computerNumber) {
    alert("WINNER");
    return true;
  } else if (userNumber > computerNumber) {
    alert(
      `Your number is bigger then the computer’s, guess again. Number of tries left: ${retries}`
    );
  } else if (userNumber < computerNumber) {
    alert(
      `Your number is smaller then the computer’s, guess again. Number of tries left: ${retries}`
    );
  }
  return false;
}
